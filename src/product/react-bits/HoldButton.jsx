...
export default function HoldButton({
  ...
  const gesture = useRef({ pointerId: null, rect: null, tap: false });
  const timers = useRef({ complete: 0, reset: 0, tap: 0 });
  const hintId = useId();
  ...
  const begin = kind => {
    ...
    gesture.current.tap = true;
    timers.current.tap = setTimeout(() => { gesture.current.tap = false; }, TAP_MS);
    go('holding', kind);
  ...
  const release = ({ drifted = false } = {}) => {
    ...
    go('idle');
    drive(0, releaseTime, EASE_OUT);
    if (!drifted && gesture.current.tap) onTap?.();
  ...
  const handlePointerDown = e => {
    ...
    } catch { /* Pointer capture is unavailable in some embedded browsers. */ }
  };
  ...
  const endPointer = (e, options) => {
    ...
    } catch { /* Pointer capture can already be released by the platform. */ }
    release(options);
  };
  ...
  useEffect(() => {
    ...
    const cancel = () => {
      ...
      go('idle');
      motion.current.p = 0;
      buttonRef.current?.style.setProperty('--hb-p', '0');
      gesture.current.tap = false;
    };
    const onVisibility = () => {
