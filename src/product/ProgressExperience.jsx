...
import { ArrowUpRight, Award, BrainCircuit, History, Scale, TrendingUp } from 'lucide-react';
...
export default function ProgressExperience() {
  ...
  const reload = () => {
    ...
    getCompletedSessions(user.id).then((value) => { setSessions(value); setRangeAnchor(Date.now()); }).catch(() => setError('Progress is unavailable right now.')).finally(() => setLoading(false));
  };
